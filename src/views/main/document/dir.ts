export const data = [
  ...
  },
  {
    label: 'public',
    des: ['一些公用不需要打包的资源'],
    children: [{ label: 'favicon.ico', des: ['网站图标'] }],
  },
  {
    label: 'src',
    des: ['网站根目录，核心目录，包含了业务代码、路由配置、状态管理、数据请求、自定义组件、自定义事件等等一系列核心的代码'],
    children: [
      {
        label: 'api',
        des: ['接口集合，用来统一管理接口数据，业务代码通过引入来进行调用'],
        children: [
          {
            label: 'system',
            des: ['系统管理模块的接口'],
            children: [{ label: 'user.ts', des: ['用户管理相关接口'] }],
          },
      ...
      },
      {
        label: 'config',
        des: ['项目核心配置文件'],
        children: [{ label: 'index.ts', des: ['核心配置文件'] }],
      },
      {
        label: 'demo',
        des: ['vue模板demo区'],
      ...
      },
      {
        label: 'directive',
        des: [
          '自定义指令集合，包含了点击外部指令、复制指令，防抖指令，拖拽指令，弹窗拖拽指令，长按指令，水印生成指令等，具体使用可参照自定义指令栏目',
        ],
        children: [
        ...
        children: [
          {
            label: 'Header',
            des: [
              '顶部功能区，左侧有收缩菜单功能、面包屑导航，右侧有全屏、选择语言、选择布局大小（针对element-plus）、主题选择、修改密码、退出登录等功能',
            ],
            children: [
              {
                label: 'functionList',
          ...
          },
          {
            label: 'Logo',
            des: ['logo展示区代码'],
            children: [{ label: 'index.vue', des: ['logo展示主文件'] }],
          },
          {
            label: 'Menu',
            des: ['左侧菜单管理区'],
          ...
          {
            label: 'Tabs',
            des: ['标签管理区，包含了keep-alive功能的主要实现区，刷新，关闭页面，路由跳转之后自动定位功能'],
            children: [
          ...
          {
            label: 'index.vue',
            des: ['layout核心文件，用于引入上述那些菜单、功能以及实现布局等功能，router文件夹里面引入的就是此文件'],
          },
        ],
      },
      {
        label: 'locale',
        des: ['国际化处理文件， 可适用各类中大型项目的国际化管理'],
        children: [
          {
            label: 'modules',
            des: ['国际化信息配置区，可基于这个文件夹扩展语言'],
            children: [
      ...
      {
        label: 'router',
        des: ['路由管理中心，可自由切换hash或history模式'],
        children: [
          {
            label: 'modules',
            des: ['核心路由配置区域，需要在index.ts内引入使用，因为自动引入有顺序问题，故不使用文件夹直接解析的模式'],
            children: [],
          },
          {
            label: 'createNode.ts',
      ...
      },
      {
        label: 'store',
        des: [
          'vuex状态管理组件,包含了本地化存储插件、分文件管理大文件等功能，已针对于中大型项目做了大量的细节适配。退出功能可直接清除所有的本地存储信息，所以如有异常，重新登录即可。',
        ],
        children: [
          {
            label: 'modules',
            des: [
              '分模块定义了各个系列的状态管理文件，不需要在index.ts内部单独引入，已做全量引入，扩展文件，可直接复制其中一个文件进行改造即可。',
            ],
            children: [],
          },
      ...
      {
        label: 'utils',
        des: ['部分通用的JS处理逻辑，其中system为系统内部的处理专用，开发者可自行建立其他文件夹来进行开发，以便区分。'],
        children: [
          {
            label: 'system',
            des: ['系统内部通用处理专用文件夹，包括了进度条处理、请求封装处理、百度统计、标题管理等。'],
            children: [
      ...
      },
      {
        ...
        children: [
          ...
          {
            label: 'system',
            des: ['系统内部处理文件，包括了404页面、401权限页面、登录页面、重定向页面等等。'],
            children: [{ label: '401.vue' }, { label: '404.vue' }, { label: 'login.vue' }, { label: 'redirect.vue' }],
          },
        ],
      },
